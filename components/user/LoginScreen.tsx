...
type Mode = "admin" | "otp-details" | "otp-verify";
...
export default function LoginScreen() {
  ...
  const sendOtp = async () => {
    ...
    if (!playerPhone.trim()) { setError("Please enter your phone number"); return; }
    ...
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        data: { full_name: playerName.trim(), phone: playerPhone.trim() },
      },
    });
    if (error) { setError(error.message); setLoading(false); return; }
    setLoading(false);
  ...
  const verifyOtp = async () => {
    ...
    // Save name + phone to profiles table
    ...
    <div className="min-h-screen bg-[#08080f] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <AnimatePresence mode="wait">
          ...
          {mode === "admin" && (
            <motion.div key="admin" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="••••••••" onKeyDown={e => e.key === "Enter" && adminLogin()}
              ...
              <button onClick={() => { setMode("otp-details"); setError(""); }}
          ...
          {mode === "otp-details" && (
            <motion.div key="otp-details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="bg-[#3b82f6]/10 border border-[#3b82f6]/25 rounded-lg p-3 mb-4 text-[11px] text-[#60a5fa] leading-relaxed">
                🎮 Enter your details — we'll send a <strong>6-digit code</strong> to your email.
              </div>

              <label className="block font-orbitron text-[9px] tracking-[2px] text-[#55556a] mb-1.5">FULL NAME</label>
              <input type="text" value={playerName} onChange={e => setPlayerName(e.target.value)}
                placeholder="Your name" autoFocus
                className="w-full bg-[#161625] border border-[#2a2a42] focus:border-[#f0b429] rounded-lg px-3 py-2.5 text-sm text-[#eeeef8] outline-none transition-colors mb-3" />

              <label className="block font-orbitron text-[9px] tracking-[2px] text-[#55556a] mb-1.5">PHONE NUMBER</label>
              <input type="tel" value={playerPhone} onChange={e => setPlayerPhone(e.target.value)}
                placeholder="+91 98765 43210"
                className="w-full bg-[#161625] border border-[#2a2a42] focus:border-[#f0b429] rounded-lg px-3 py-2.5 text-sm text-[#eeeef8] outline-none transition-colors mb-3" />

              <label className="block font-orbitron text-[9px] tracking-[2px] text-[#55556a] mb-1.5">EMAIL ADDRESS</label>
              <input type="email" value={playerEmail} onChange={e => setPlayerEmail(e.target.value)}
                placeholder="blader@arena.gg" onKeyDown={e => e.key === "Enter" && sendOtp()}
          ...
          {mode === "otp-verify" && (
            <motion.div key="otp-verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="text-center mb-5">
                ...
                <div className="text-[12px] text-[#eeeef8] font-semibold mt-0.5">{playerEmail}</div>
              </div>
              ...
              <div className="flex items-center justify-between mt-3">
                <button onClick={() => { setMode("otp-details"); setError(""); setOtpCode(""); }}
